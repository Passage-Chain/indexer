...
export class NodeInfo {
  ...
  public isAvailable(height?: number): boolean {
    return this.status === NodeStatus.OK && (!height || (!!this.earliestBlockHeight && this.earliestBlockHeight <= height)) && this.activeQueries.length < this.maxConcurrentQuery;
  }
  ...
  public async query(path: string, height?: number) {
    ...
    try {
      ...
      if (err.response?.status === HttpCodes.TOO_MANY_REQUESTS) {
        ...
        if(!rpcError) throw "Unknown error"
